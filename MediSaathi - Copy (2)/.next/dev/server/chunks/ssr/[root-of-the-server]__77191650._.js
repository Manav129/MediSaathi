...
function AuthProvider({ children }) {
    ...
    const fetchUserData = async (email)=>{
        try {
            console.log(email);
            const { data: userData, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('*').eq('email', email).single();
            if (error) {
    ...
    const updateUserType = async (userType)=>{
        if (!user) return;
        try {
            const { error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').update({
                user_type: userType,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__77191650._.js.map